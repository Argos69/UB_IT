...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Статус = Перечисления.УБ_СтатусыМоделейПланирования.Неактуальный 
		или Объект.Статус = Перечисления.УБ_СтатусыМоделейПланирования.Закрыто Тогда
		
		ЭтаФорма.ТолькоПросмотр = Истина;
	КонецЕсли;	
	
	Элементы.Статус.СписокВыбора.Очистить();
	Элементы.Статус.СписокВыбора.Добавить(Перечисления.УБ_СтатусыМоделейПланирования.Планируется);
	Элементы.Статус.СписокВыбора.Добавить(Перечисления.УБ_СтатусыМоделейПланирования.Действует);
	Если Объект.Статус = Перечисления.УБ_СтатусыМоделейПланирования.Неактуальный Тогда
		Элементы.Статус.СписокВыбора.Добавить(Перечисления.УБ_СтатусыМоделейПланирования.Неактуальный);
	КонецЕсли;
	Если Объект.Статус = Перечисления.УБ_СтатусыМоделейПланирования.Закрыто Тогда
		Элементы.Статус.СписокВыбора.Добавить(Перечисления.УБ_СтатусыМоделейПланирования.Закрыто);
	КонецЕсли;
	...
	Если Константы.УБ_ИспользоватьБезокладнуюСистемуПоказателей.Получить() Тогда
		Если Константы.УБ_СпособРаспределенияПремии.Получить() = Перечисления.УБ_СпособРаспределенияПремии.Стандартный 
			ИЛИ Не ЗначениеЗаполнено(Константы.УБ_СпособРаспределенияПремии.Получить()) Тогда
		
			Элементы.ПредельныйКоэффициент.Видимость = Ложь;
		Иначе 	
			Элементы.ПредельныйКоэффициент.Видимость = Истина;
		КонецЕсли;	
	КонецЕсли;
	//Новое ТАА
...
Процедура СтатусПриИзмененииЗавершение()
	...
	//Если Объект.Статус = ПредопределенноеЗначение("Перечисление.УБ_СтатусыМоделейПланирования.Действует") Тогда
	//	Если ИспользуютсяНесколькоМатричныхВДереве() Тогда
	//		Объект.Статус = ПредопределенноеЗначение("Перечисление.УБ_СтатусыМоделейПланирования.Неактуальный");
	//		Сообщить("В действующей модели должно быть не больше 1 матричного типа показателей");
	//	КонецЕсли;	
	//КонецЕсли;	
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.УБ_СтатусыМоделейПланирования.Действует")
		И Объект.ДатаУтверждения = '00010101' Тогда
...
Процедура ГрейдЗакрытПриИзмененииНаСервере()
	...
	Иначе 
		...
		НомерСтатусаЗакрыто = Элементы.Статус.СписокВыбора.НайтиПоЗначению(Перечисления.УБ_СтатусыМоделейПланирования.Закрыто);
...
Процедура НайтиДругиеУтвержденныеВерсииГрейда()
	...
	Запрос.Текст = 
		...
		|		МодельПланированияЭффективности = &МодельПланированияЭффективности
		|			И Грейд = &Грейд
		|			И Статус = ЗНАЧЕНИЕ(Перечисление.УБ_СтатусыМоделейПланирования.Действует)) КАК ВерсииГрейдовСрезПервых";
	...
	Элементы.ПредыдущаяВерсияГрейда.Видимость = ЗначениеЗаполнено(ПредыдущаяВерсияГрейда);
	Элементы.СледующаяВерсияГрейда.Видимость = ЗначениеЗаполнено(СледующаяВерсияГрейда);
...
Процедура УстановитьДоступностьЭлементовФормы(Форма)
	...
	Иначе	
		...
		//	"ПодборПоказателейВДерево",
		//	"Доступность",
		//	Не Форма.ДеревоПустое И Не ГрейдУтвержден);	
	...
	Если Параметр.ОтобранныеПоказатели.Количество() > 0 Тогда
		...
		Если ТипЗнч(Параметр.ОтобранныеПоказатели[0]) = Тип("СправочникСсылка.УБ_ПоказателиЭффективности") Тогда
			РеквизитыПоказателей = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(Параметр.ОтобранныеПоказатели,
												"ВидРасценки, ЕдиницаИзмерения, Наименование, ТипПоказателя");
