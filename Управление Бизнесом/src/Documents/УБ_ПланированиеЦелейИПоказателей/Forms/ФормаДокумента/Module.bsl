...
Процедура НазначенияПриАктивизацииСтроки(Элемент)
	...
	Если НЕ ТекущиеДанные = Неопределено и НЕ ТекущиеДанные.Назначение = Неопределено Тогда// И НЕ ТекущиеДанные.Назначение = "Компания"  Тогда
		Элементы.ГруппаПоказатели.Заголовок = "Показатели "+ УБ_ОбщегоНазначенияКлиентВызовСервера.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Назначение, "Представление");	
	Иначе   
		СтрОтбора = Новый ФиксированнаяСтруктура("КлючСвязи", Неопределено);
		ЭтотОбъект.Элементы["Показатели"].ОтборСтрок = СтрОтбора;
		Элементы.ГруппаПоказатели.Заголовок = "показатели";
	КонецЕсли;  
...
Процедура ПодготовитьФормуНаСервере() Экспорт
	...
	УстановитьСвойстваКнопкиУтвержден(Элементы.ФормаУтверждена, Объект.Утвержден);	
	Компания = ЗначениеРеквизитаПоУмолчанию("Компания");
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(ИмяТаблицы = "") Экспорт
	...
	Подразделения = Объект.Назначения.НайтиСтроки(Новый Структура("НазначениеТип", ЗначениеРеквизитаПоУмолчанию("Подразделение")));
	...
	//для Проекта
	Проекты = Объект.Назначения.НайтиСтроки(Новый Структура("НазначениеТип", ЗначениеРеквизитаПоУмолчанию("Проект")));
...
Процедура УстановитьОтборСтрокДляНазначений(Элемент)
	...
	СтрОтбора = Новый ФиксированнаяСтруктура("НазначениеТип", ЗначениеРеквизитаПоУмолчанию(ТекущееНазначение));
...
Процедура УстановитьЗаголовокКолонкиНазначение(Назначение)                      
	...
	Количество = Объект.Назначения.НайтиСтроки(Новый Структура("НазначениеТип", ЗначениеРеквизитаПоУмолчанию(Назначение))).Количество();		
	Элементы[Назначение].Заголовок = ?(Количество = Неопределено, Назначение, Назначение+"("+Количество+")");
	Элементы.Назначения.ТолькоПросмотр = Назначение = "Компания";      
...
Процедура СкопироватьпоказательВсемНаСервере(НомерСтроки)  
	...
	МассивНазначенийВида = Объект.Назначения.НайтиСтроки(Новый Структура("НазначениеТип", ЗначениеРеквизитаПоУмолчанию(ТекущееНазначение)));
...
Процедура НазначенияЦельНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;                                                       
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СтруктураПредприятия", Объект.СтруктураПредприятия);
	ОткрытьФорму("Справочник.УБ_Цели.Форма.ФормаСписка", ПараметрыФормы, Элемент,,,,);
