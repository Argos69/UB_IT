...
Процедура ПодборСотрудниковНаСервере(СИерархией = Ложь)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	УБ_ДанныеДляПодбораСотрудников.Организация = &Организация
	               |	И УБ_ДанныеДляПодбораСотрудников.Начало <= &Окончание
	               |	И УБ_ДанныеДляПодбораСотрудников.Окончание >= &Начало";
	Если СИерархией Тогда
		Запрос.Текст = Запрос.Текст +" И УБ_ДанныеДляПодбораСотрудников.Подразделение В Иерархии (&Подразделение)";
		Запрос.УстановитьПараметр("Подразделение",Подразделение);
	Иначе	
		Запрос.Текст = Запрос.Текст +" И УБ_ДанныеДляПодбораСотрудников.Подразделение = &Подразделение";
		Запрос.УстановитьПараметр("Подразделение",Подразделение);
	КонецЕсли;
	Запрос.УстановитьПараметр("Организация",Организация);
	Запрос.УстановитьПараметр("Начало",НачалоМесяца(ПериодФормирования));
	Запрос.УстановитьПараметр("Окончание",КонецМесяца(ПериодФормирования)); 
